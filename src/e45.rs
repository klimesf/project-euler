...
fn calc() -> usize {
    ...
    loop {
        ...
        if triangle_n < sieve_triangle.len() { sieve_triangle[triangle_n] = true }
        let pentagonal_n = n * (3 * n - 1) / 2;
        if pentagonal_n < sieve_pentagonal.len() { sieve_pentagonal[pentagonal_n] = true }
        let hexagonal_n = n * (2 * n - 1);
        if hexagonal_n < sieve_hexagonal.len() { sieve_hexagonal[hexagonal_n] = true }

        if triangle_n >= sieve_triangle.len() && pentagonal_n >= sieve_pentagonal.len() && hexagonal_n >= sieve_hexagonal.len() {
            break;
        }
